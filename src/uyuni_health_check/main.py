...
def show_uyuni_live_server_metrics(metrics: dict):
    """
...
def show_supportconfig_metrics(metrics: dict):
    if metrics:
        tables = []
        tables.append(show_salt_jobs_summary(metrics))
...
def show_error_logs_stats(loki, since):
    """
...
def show_full_error_logs(loki, since):
    """
...
def run(ctx, exporter_port, loki, logs, since, clean):
    ...
    try:
        ...
        if supportconfig_path:
            show_supportconfig_metrics(metrics)
        else:
            show_uyuni_live_server_metrics(metrics)

        show_relevant_hints()
        console.print(Markdown("## Relevant Errors"))
        loki_url = loki if loki else f"http://{host}:3100"
        show_error_logs_stats(loki_url, since)
        if logs:
            show_full_error_logs(loki_url, since)
...
def _fetch_metrics_from_exporter(host="localhost", port=9000, max_retries=5):
...
def fetch_metrics_from_supportconfig_exporter(
    host="localhost", port=9000, max_retries=5
):
    if not host:
        host = "localhost"

    metrics_raw = _fetch_metrics_from_exporter(host, port, max_retries)
